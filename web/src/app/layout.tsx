...
const crmHosts = new Set(
  crmHostCandidates.map((value) => value.toLowerCase().split(":")[0]!),
);

const mainSiteUrl =
  process.env.NEXT_PUBLIC_MAIN_SITE_URL?.trim() || "https://deckd.us";

function isCrmHost(hostHeader: string | null): boolean {
  if (!hostHeader) {
    return false;
  }
  const normalized = hostHeader.toLowerCase().split(":")[0] ?? "";
  if (crmHosts.has(normalized)) {
    return true;
  }
              ...
              <div className="mx-auto flex max-w-5xl items-center justify-between gap-6 px-6 py-5 md:px-10">
                {isCrm ? (
                  <Link href="/crm" className="flex items-center gap-3 text-slate-100">
                    <span className="flex h-10 w-10 items-center justify-center rounded-2xl border border-sky-300/60 bg-sky-400/20 text-sm font-semibold tracking-[0.35em] text-sky-200">
                      CRM
                    </span>
                    <span className="hidden text-sm font-medium uppercase tracking-[0.4em] text-sky-100 sm:block">
                      deckd studio ops
                    </span>
                  </Link>
                ) : (
                ...
                {isCrm ? (
                  <nav className="flex flex-wrap items-center gap-3 text-xs font-semibold uppercase tracking-[0.35em] text-slate-300">
                    <Link
                      href="/crm"
                      className="rounded-full border border-transparent px-4 py-2 transition hover:border-sky-300/60 hover:bg-sky-300/10 hover:text-sky-100"
                    >
                      Dashboard
                    </Link>
                    <a
                      href={mainSiteUrl}
                      className="rounded-full border border-transparent px-4 py-2 transition hover:border-amber-300/60 hover:bg-amber-300/10 hover:text-amber-100"
                    >
                      Main site
                    </a>
                  </nav>
              ...
              <div className="mx-auto flex max-w-5xl flex-col gap-3 px-6 py-6 text-xs text-slate-400 md:flex-row md:items-center md:justify-between md:px-10">
                ...
                {isCrm ? (
                  <div className="flex flex-wrap gap-3">
                    <Link
                      href="/crm"
                      className="rounded-full border border-white/10 px-3 py-1 text-[0.7rem] text-slate-200 transition hover:border-sky-300/60 hover:bg-sky-300/10 hover:text-sky-100"
                    >
                      CRM dashboard
                    </Link>
                    <a
                      href={mainSiteUrl}
                      className="rounded-full border border-white/10 px-3 py-1 text-[0.7rem] text-slate-200 transition hover:border-amber-300/60 hover:bg-amber-300/10 hover:text-amber-100"
                    >
                      deckd.us
                    </a>
                  </div>
